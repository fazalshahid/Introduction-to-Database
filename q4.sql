...
create view election_comb_not_first as
	select * from election_comb where prev_id is NOT NULL;

select count(*) from election_comb;
select count(*) from election_comb_not_first;

CREATE VIEW RepeatParty as
SELECT e1.country,count(*) as num_repeat_party
FROM election_comb_not_first e1, election_comb e2
WHERE e1.prev_id=e2.election_id AND e1.party_id=e2.party_id and e1.res_id<>e2.res_id and e1.country = e2.country and (e2.election_id,e2.party_id) in (select election_id,party_id from election_winners) and (e1.election_id,e1.party_id) in (select election_id,party_id from election_winners)
GROUP BY e1.country;

SELECT e1.country,e1.election_id,e1.prev_id,e1.party_id,e2.country,e2.election_id,e2.prev_id,e2.party_id
FROM election_comb_not_first e1, election_comb e2
WHERE e1.prev_id=e2.election_id AND e1.party_id=e2.party_id and e1.res_id<>e2.res_id and e1.country = e2.country and (e2.election_id,e2.party_id) in (select election_id,party_id from election_winners) and (e1.election_id,e1.party_id) in (select election_id,party_id from election_winners)
;
...
create view cab_comb as
	select distinct country.name as country,electoral_system,election_id,party_id,election.previous_parliament_election_id as prev_id
	from election_winners,election,country
	where election_winners.election_id = election.id and election.country_id=country.id and election.e_type='Parliamentary election';

create view cab_comb_not_first as
	select * from cab_comb where prev_id is NOT NULL;



CREATE VIEW RepeatParty as
SELECT e1.country,count(*) as num_repeat_party
FROM cab_comb_not_first e1, cab_comb e2
WHERE e1.prev_id=e2.election_id AND e1.party_id=e2.party_id and e1.election_id!=e2.election_id and  e1.country = e2.country 
GROUP BY e1.country;
*/
...
/*
CREATE VIEW election_comb as 
SELECT country.name as country, election_id, alliance_id,party_id,election_result.id as res_id,election.previous_parliament_election_id as prev_id
FROM election,election_result,country
WHERE election.id=election_result.election_id and election.country_id=country.id and election.e_type='Parliamentary election';
...
select country,election_id, res_id as alliance_id,party_id,res_id,prev_id
