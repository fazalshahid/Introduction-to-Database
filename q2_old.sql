...
CREATE VIEW all_winners as
SELECT country.name as country,electoral_system,election.id AS election_id,election_result.party_id
FROM election_winners JOIN election_result on election_winners.election_id=election_result.election_id and election_winners.party_id=election_result.party_id JOIN election on election_result.election_id=election.id
	JOIN country ON country.id=election.country_id
where election.e_type='Parliamentary election';
...
SELECT country,electoral_system,alliance_id,election_id,res_id,party_id
from aw
where alliance_id IS NOT NULL or(alliance_id is NULL AND res_id in (select  alliance_id as res_id from aw));  
...
CREATE VIEW t as
	select country,electoral_system,election_id,count(*) as single_party,count(alliance_id) as al,count(res_id) as res,count(party_id) as p
	from no_alliance
	group by country,electoral_system,election_id;
select * from t;


select count(distinct election_id),count(res_id),count(party_id),count(distinct res_id),count(distinct party_id),count(alliance_id) from all_winners;
select count(distinct election_id),count(res_id),count(party_id),count(distinct res_id),count(distinct party_id),count(alliance_id) from aw;

--select count(*) from alliance_parties;



--select count(*) from all_winners;

--SELECT * from no_alliance;

--select alliance_id as election_id from all_winners;
--select * from alliance_parties;
