...
WHERE e1.prev_id=e2.election_id AND e1.party_id=e2.party_id and e1.res_id!=e2.res_id and (e1.prev_id,e2.party_id) in (select election_id,party_id from election_winners) and (e1.election_id,e1.party_id) in (select election_id,party_id from election_winners)
GROUP BY e1.country;
